...
// int __aarch64_cas8_acq_rel() {
//     return 0;
// }

// int __aarch64_swp8_acq_rel() {
//     return 0;
// }
...
init(void)
{
    ...
    // double input = 1;
    // double out = input << 3;
    // printf("shifting 0x%08x left %d: 0x%08x\n", (int)input, 3, (int)(input << (double)3));
    ...
    char *mallocd_ptr = malloc(strlen(hello) * 1000);
    if (!mallocd_ptr) {
        printf("???\n");
    ...
    printf("here\n");
    printf("%p\n", mallocd_ptr);
    memcpy(mallocd_ptr, hello, strlen(hello));
    ...
    char *mallocd_ptr2 = malloc(strlen(hello));
    printf("%p\n", mallocd_ptr2);
